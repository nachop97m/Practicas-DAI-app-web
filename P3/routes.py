...
def login():
    ...
    db = PickleShareDB('/vagrant/Practicas/P3/p3.db')
...
def models():
    ...
    if 'user' in session:
        ...
        return render_template('modelS.html', user = session['user'], session=session)
...
def modelx():
    ...
    if 'user' in session:
        session['pagina3'] = session['pagina2']
        session['pagina2'] = session['pagina1']
        session['pagina1'] = 'modelx'
        session['pagina3text'] = session['pagina2text']
        session['pagina2text'] = session['pagina1text']
        session['pagina1text'] = 'TESLA MODEL X'

        return render_template('modelX.html', user = session['user'], session=session)
...
def model3():
    ...
    if 'user' in session:
        session['pagina3'] = session['pagina2']
        session['pagina2'] = session['pagina1']
        session['pagina1'] = 'model3'
        session['pagina3text'] = session['pagina2text']
        session['pagina2text'] = session['pagina1text']
        session['pagina1text'] = 'TESLA MODEL 3'
            
        return render_template('model3.html', user = session['user'], session=session)
